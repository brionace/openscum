...
export default async function HomePage() {
  try {
    ...
    try {
      ...
      throw new Error(`Reports fetch failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
      ...
      throw new Error(`Stats fetch failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
      ...
      throw new Error(`Outcome types fetch failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center max-w-2xl">
          ...
          <p className="text-gray-600 mb-4">Unable to load the page. Please check the server logs.</p>
