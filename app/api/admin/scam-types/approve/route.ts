...
// POST /api/admin/scam-types/approve - Approve a scam type
export async function POST(request: NextRequest) {
  try {
    ...
    // Update the scam type to approved using raw SQL
    await prisma.$executeRaw`
      UPDATE "ScamType" 
      SET "isApproved" = true, "moderatedAt" = NOW(), "moderatedBy" = ${user.id}
      WHERE id = ${scamTypeId}
    `;

    return NextResponse.json({ success: true, message: "Scam type approved" });
  } catch (error) {
    console.error("Error approving scam type:", error);
    return NextResponse.json(
      { error: "Failed to approve scam type" },
      { status: 500 }
    );
  }
}
